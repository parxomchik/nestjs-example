...
## Running the API
### Development
To start the application locally you need MongoDB 
and Redis started locally. You could use [Docker](#docker) for
this purposes.
...
#### For start local MongoDB and Redis environment with Docker:
``` 
  docker-compose -f docker-compose.local.yml up
```

#### For start production application with Docker:
```
...
docker-compose -f "./docker-compose.local.yml" -p podil-api up -d --build --remove-orphans
